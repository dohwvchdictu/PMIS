<div class="space-y-2">
    ...
    <div>
        @if ($activeTab == 1)
            <div class="flex flex-col gap-2 pt-2">
                    ...
                    <div class="flex items-center justify-between mb-4">
                        <div>
                            ...
                            <p class="text-sm text-gray-600 dark:text-gray-400">
                                Managing {{ $procurements->count() }} PR{{ $procurements->count() > 1 ? 's' : '' }}
                                together
                            </p>
                        </div>
                        <span
                            class="text-xs bg-emerald-200 dark:bg-emerald-800 text-emerald-800 dark:text-emerald-200 px-3 py-1.5 rounded-full font-semibold">
                            Grouped Procurement
                        </span>
                    </div>

                    {{-- List of PRs in Group --}}
                    <div class="bg-gray-50 dark:bg-neutral-800 rounded-lg p-3">
                        <h4 class="text-xs font-semibold text-gray-700 dark:text-gray-300 mb-2">PRs in this group:</h4>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-2">
                            @foreach ($form['procurements'] ?? [] as $proc)
                                <div
                                    class="flex items-center gap-2 text-xs bg-white dark:bg-neutral-700 p-2 rounded border border-gray-200 dark:border-neutral-600">
                                    <span
                                        class="font-mono font-bold text-emerald-600 dark:text-emerald-400">{{ $proc['pr_number'] }}</span>
                                    <span
                                        class="text-gray-600 dark:text-gray-400 truncate flex-1">{{ Str::limit($proc['procurement_program_project'], 50) }}</span>
                                    <span
                                        class="text-gray-500 dark:text-gray-400 whitespace-nowrap">₱{{ number_format($proc['abc'] ?? 0, 2) }}</span>
                                </div>
                            @endforeach
                        </div>
                    </div>
                ...
                {{-- Schedule Table (Same as per-lot) --}}
                <div
                            ...
                            <tbody class="divide-y divide-gray-200 dark:divide-neutral-800">
                                @forelse(array_reverse($form['items'] ?? [], true) as $itemIndex => $item)
                                    ...
                                    <tr wire:key="row-{{ $rowUid }}"
                                        ...
                                        {{-- Bidding Fields --}}
                                        @if ($showFields && $modeId && !in_array($modeId, array_merge([1], range(7, 24))))
                                            <td class="px-2 py-2">
                                                <input type="text" wire:key="bid-num-{{ $rowUid }}"
                                                    ...
                                                    {{ $errors->has('form.items.' . $itemIndex . '.bidding_number') ? 'border-red-500 focus:ring-red-500 focus:border-red-500' : 'border-gray-300 dark:border-neutral-600 focus:ring-emerald-500' }}"
                                            ...
                                            <td class="px-2 py-2">
                                                <input type="text" wire:key="ib-{{ $rowUid }}"
                                                    ...
                                                    class="w-full px-2 py-1 text-xs text-right border rounded focus:ring-2 dark:bg-neutral-800 dark:text-white disabled:opacity-60 disabled:cursor-not-allowed
                                                    {{ $errors->has('form.items.' . $itemIndex . '.ib_number') ? 'border-red-500 focus:ring-red-500 focus:border-red-500' : 'border-gray-300 dark:border-neutral-600 focus:ring-emerald-500' }}"
                                        ...
                                        {{-- SVP Fields --}}
                                        @if ($showFields && in_array($modeId, range(7, 24)))
                                            <td class="px-2 py-2">
                                                <input type="text" wire:key="rfq-{{ $rowUid }}"
                                                    wire:model.defer="form.items.{{ $itemIndex }}.rfq_no"
                                                    class="w-full px-2 py-1 text-xs text-right border rounded focus:ring-2 dark:bg-neutral-800 dark:text-white disabled:opacity-60 disabled:cursor-not-allowed
                                                    {{ $errors->has('form.items.' . $itemIndex . '.rfq_no') ? 'border-red-500 focus:ring-red-500' : 'border-gray-300 dark:border-neutral-600 focus:ring-emerald-500' }}"
                                            ...
                                            <td class="px-2 py-2">
                                                <input type="date" wire:key="can-date-{{ $rowUid }}"
                                                    ...
                                                    {{ $errors->has('form.items.' . $itemIndex . '.canvass_date') ? 'border-red-500 focus:ring-red-500' : 'border-gray-300 dark:border-neutral-600 focus:ring-emerald-500' }}"
                                                    @disabled($disableInputs)>
                                            </td>
                                            <td class="px-2 py-2">
                                                <input type="date" wire:key="ret-can-{{ $rowUid }}"
                                                    ...
                                                    class="w-full px-2 py-1 text-xs border rounded focus:ring-2 dark:bg-neutral-800 dark:text-white disabled:opacity-60 disabled:cursor-not-allowed
                                                    {{ $errors->has('form.items.' . $itemIndex . '.date_returned_of_canvass') ? 'border-red-500 focus:ring-red-500' : 'border-gray-300 dark:border-neutral-600 focus:ring-emerald-500' }}"
                                                    @disabled($disableInputs)>
                                            </td>
                                            <td class="px-2 py-2">
                                                <input type="date" wire:key="abs-can-{{ $rowUid }}"
                                                    ...
                                                    class="w-full px-2 py-1 text-xs border rounded focus:ring-2 dark:bg-neutral-800 dark:text-white disabled:opacity-60 disabled:cursor-not-allowed
                                                    {{ $errors->has('form.items.' . $itemIndex . '.abstract_of_canvass_date') ? 'border-red-500 focus:ring-red-500' : 'border-gray-300 dark:border-neutral-600 focus:ring-emerald-500' }}"
                                                    @disabled($disableInputs)>
                                            </td>
                                            <td class="px-2 py-2">
                                                <input type="text" wire:key="res-num-{{ $rowUid }}"
                                                    ...
                                                    class="w-full px-2 py-1 text-xs text-right border rounded focus:ring-2 dark:bg-neutral-800 dark:text-white disabled:opacity-60 disabled:cursor-not-allowed
                                                    {{ $errors->has('form.items.' . $itemIndex . '.resolution_number') ? 'border-red-500 focus:ring-red-500' : 'border-gray-300 dark:border-neutral-600 focus:ring-emerald-500' }}"
    ...
    {{-- Edit History Modal --}}
    <x-forms.modal wire:model="showModal" title="Edit History" size="max-w-6xl">
        @if ($editingItem)
            <div class="px-4 py-3">
                @php
                        ...
                        <thead class="sticky top-0 bg-gray-100 dark:bg-neutral-700 z-10">
                            <tr>
                                <th
                                    ...
                                    Mode of Procurement</th>
                                @if ($editModeId && !in_array($editModeId, array_merge([1], range(7, 24))))
                                    <th
                                        class="px-2 py-2 text-left font-semibold text-black dark:text-white border-b border-gray-300 dark:border-neutral-600 w-20">
                                        Bidding #</th>
                                    <th
                                    ...
                                    <th
                                        ...
                                        Bidding Result</th>
                                @endif
                                @if ($editModeId && in_array($editModeId, range(7, 24)))
                                    <th
                                        class="px-2 py-2 text-left font-semibold text-black dark:text-white border-b border-gray-300 dark:border-neutral-600">
                                        RFQ No.</th>
                                    <th
