    ...
    <x-forms.modal :model="'showPostBulkEditModal'" :closeMethod="'closePostBulkEditModal'" :title="'Bulk Edit Post Procurement'" size="max-w-6xl">
        @if (!empty($postBulkEditData))
            <div class="overflow-x-auto max-h-[600px] overflow-y-auto">
                ...
                <!-- Selected Items List -->
                <div class="mb-4">
                    <div class="px-2 py-3 bg-blue-50 dark:bg-blue-900/20 border-b-2 border-blue-500 mb-2">
                    ...
                    <table class="w-full text-xs">
                        <thead class="bg-gray-200 dark:bg-neutral-800">
                            <tr>
                                <th class="px-4 py-2 text-left font-semibold text-black dark:text-white">PR Number</th>
                                <th class="px-4 py-2 text-left font-semibold text-black dark:text-white">Procurement
                                    Program / Project</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-gray-200 dark:divide-neutral-800">
                            @if (!empty($postBulkEditData['selected_items']))
                                @foreach ($postBulkEditData['selected_items'] as $item)
                                    <tr class="hover:bg-gray-50 dark:hover:bg-neutral-700">
                                        <td class="px-4 py-2 text-gray-900 dark:text-white">
                                            {{ $item['pr_number'] ?? '' }}</td>
                                        <td class="px-4 py-2 text-gray-600 dark:text-gray-400">
                                            {{ $item['procurement_program_project'] ?? '' }}</td>
                                    </tr>
                                @endforeach
                            @endif
                        </tbody>
