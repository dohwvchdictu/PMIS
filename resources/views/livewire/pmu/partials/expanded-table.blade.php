<table class="min-w-full divide-y divide-gray-200 dark:divide-neutral-700 rounded-lg overflow-hidden">
    ...
    <tbody class="bg-white divide-y divide-gray-200 dark:bg-neutral-800 dark:divide-neutral-700">
            ...
            <tr class="hover:bg-gray-50 dark:hover:bg-neutral-700">
                <td class="px-4 py-3 whitespace-nowrap text-sm font-medium text-emerald-700 dark:text-emerald-300">
                    @can('view_procurement')
                        <a href="{{ route('procurements.view', ['procurement' => $procRow->procID]) }}" target="_blank"
                ...
                <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-900 dark:text-white max-w-[14rem]">
                    <div class="font-medium truncate" title="{{ $procRow->description }}">
                ...
                <td class="px-4 py-3 text-sm text-gray-700 dark:text-gray-300 min-w-[16rem]">
                ...
                <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-700 dark:text-gray-300">
                    @php
                        ...
                        if ($procRow->po_date_deadline) {
                            $deadline = \Carbon\Carbon::parse($procRow->po_date_deadline);
                            $today = \Carbon\Carbon::today();
                            $daysUntil = $today->diffInDays($deadline, false);
                            if ($procRow->po_date && \Carbon\Carbon::parse($procRow->po_date)->gt($deadline)) {
                                $deadlineWarning = 'exceeded';
                            } elseif ($daysUntil < 0 && !$procRow->po_date) {
                                $deadlineWarning = 'overdue';
                            } elseif ($daysUntil >= 0 && $daysUntil <= 3 && !$procRow->po_date) {
                ...
                <td class="px-4 py-3 whitespace-nowrap text-sm">
                    @if ($procRow->po_contract_number)
                        @if ($procRow->po_contract_number_link)
                            <a href="{{ $procRow->po_contract_number_link }}" target="_blank" rel="noopener noreferrer"
                ...
                <td class="px-4 py-3 min-w-[13rem]">
                    @php
                        ...
                        $isRowComplete =
                            ...
                            !empty($procRow->pmu_date_po_receipt_by_supplier ?? null) &&
                            !empty($procRow->pmu_date_coa_stamped_received ?? null);
                        ...
                        if ($hasPoDate && $hasPoNumber && $hasContractAmount) {
                            ...
                            $autoStatusClass = 'bg-amber-100 text-amber-700 dark:bg-amber-900/40 dark:text-amber-300';
                        } else {
                ...
                <td class="px-4 py-3 whitespace-nowrap text-sm">
                    @if ($procRow->ntp_link)
                        <a href="{{ $procRow->ntp_link }}" target="_blank" rel="noopener noreferrer"
                ...
                <td class="px-4 py-3 text-sm text-gray-700 dark:text-gray-300 min-w-[16rem] w-60">
        ...
        @empty
            <tr>
                <td colspan="16" class="px-4 py-6 text-center text-sm text-gray-500 dark:text-gray-400">
                    No items found.
                </td>
            </tr>
        @endforelse
