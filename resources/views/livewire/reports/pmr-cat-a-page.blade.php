            ...
            <div class="flex items-end gap-2 flex-wrap">
                ...
                <div class="shrink-0">
                    ...
                    <input type="number" wire:model.live="year" min="2000" max="2099"
                        class="w-24 px-2.5 py-2 text-sm border border-gray-300 rounded-lg bg-white focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent dark:bg-neutral-800 dark:text-white dark:border-neutral-600" />
                </div>
            ...
            <tbody class="bg-white dark:bg-neutral-800">
                ...
                @forelse ($procurements as $procurement)
                    @php
                        ...
                        $latestMop = $isPerLot
                            ? $procurement->mopLots->sortByDesc('mode_order')->first()
                            : null;
                        ...
                        $biddingGroup = $bidScheduleMap->get($key);
                        $bidding1     = $biddingGroup?->get(1);
                        $bidding2     = $biddingGroup?->get(2);
                        $mainBid      = $bidding1 ?? $biddingGroup?->first();

                        $prSvp  = $prSvpMap->get($key);
                        $pmuPo  = $pmuPoMap->get($procurement->procID);
                    ...
                    @endphp
                    <tr class="border-b border-gray-100 dark:border-neutral-700 {{ $loop->even ? 'bg-gray-50/50 dark:bg-neutral-900/50' : 'bg-white dark:bg-neutral-800' }} hover:bg-gradient-to-r hover:from-emerald-50 hover:to-teal-50 dark:hover:from-emerald-900/20 dark:hover:to-teal-900/20 transition-all duration-200">
                        {{-- 1 PR Number --}}
                        <td class="px-3 py-2 text-center text-xs font-bold text-emerald-700 dark:text-emerald-300 whitespace-nowrap">
                            <span class="inline-flex items-center px-2 py-0.5 bg-emerald-50 dark:bg-emerald-900/30 border border-emerald-200 dark:border-emerald-700 rounded-md font-mono text-xs whitespace-nowrap">
                                {{ $procurement->pr_number }}
                            </span>
                        </td>
                        {{-- 2 IB Number --}}
                        <td class="px-3 py-2 text-xs text-center text-gray-700 dark:text-gray-300 whitespace-nowrap">{{ $isBidding ? ($mainBid?->ib_number ?? '') : '' }}</td>
                        {{-- 3 NP No. (future) --}}
                        <td class="px-3 py-2 text-xs text-gray-400"></td>
                        {{-- 4 Procurement Program/Project --}}
                        <td class="px-3 py-2 text-xs text-left text-gray-700 dark:text-gray-300">
                            <div class="break-words whitespace-normal line-clamp-2" title="{{ $procurement->procurement_program_project }}">{{ $procurement->procurement_program_project }}</div>
                        </td>
                        {{-- 5 Date Receipt --}}
                        <td class="px-3 py-2 text-xs text-center text-gray-700 dark:text-gray-300 whitespace-nowrap">{{ $fmt($procurement->date_receipt) }}</td>
                        {{-- 6 RBAC/SBAC --}}
                        <td class="px-3 py-2 text-xs text-center text-gray-700 dark:text-gray-300 whitespace-nowrap">{{ $procurement->category?->bacType?->abbreviation ?? '' }}</td>
                        {{-- 7 DTRACK # --}}
                        <td class="px-3 py-2 text-xs text-center text-gray-700 dark:text-gray-300 whitespace-nowrap">{{ $procurement->dtrack_no }}</td>
                        {{-- 8 UniCode --}}
                        <td class="px-3 py-2 text-xs text-center text-gray-700 dark:text-gray-300 whitespace-nowrap">{{ $procurement->unicode }}</td>
                        {{-- 9 Division --}}
                        <td class="px-3 py-2 text-xs text-left text-gray-700 dark:text-gray-300">
                            <div class="break-words whitespace-normal line-clamp-2" title="{{ $procurement->division?->divisions }}">{{ $procurement->division?->divisions ?? '' }}</div>
                        </td>
                        {{-- 10 Cluster/Committee --}}
                        <td class="px-3 py-2 text-xs text-left text-gray-700 dark:text-gray-300">
                            <div class="break-words whitespace-normal line-clamp-2" title="{{ $procurement->clusterCommittee?->clustercommittee }}">{{ $procurement->clusterCommittee?->clustercommittee ?? '' }}</div>
                        </td>
                        {{-- 11 Category --}}
                        <td class="px-3 py-2 text-xs text-left text-gray-700 dark:text-gray-300">
                            <div class="break-words whitespace-normal line-clamp-2">{{ $procurement->category?->category ?? '' }}</div>
                        </td>
                        {{-- 12 Venue (Specific) --}}
                        <td class="px-3 py-2 text-xs text-left text-gray-700 dark:text-gray-300">
                            <div class="break-words whitespace-normal line-clamp-2">{{ $procurement->venueSpecific?->name ?? '' }}</div>
                        </td>
                        {{-- 13 Venue (Province/HUC) --}}
                        <td class="px-3 py-2 text-xs text-left text-gray-700 dark:text-gray-300">
                            <div class="break-words whitespace-normal line-clamp-2">{{ $procurement->venueProvincesHUC?->province_huc ?? '' }}</div>
                        </td>
                        {{-- 14 Category/Venue --}}
                        <td class="px-3 py-2 text-xs text-left text-gray-700 dark:text-gray-300">
                            <div class="break-words whitespace-normal line-clamp-2">{{ $procurement->category_venue }}</div>
                        </td>
                        {{-- 15 w/Approved PPMP --}}
                        <td class="px-3 py-2 text-xs text-center text-gray-700 dark:text-gray-300">
                            <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-semibold {{ $procurement->approved_ppmp ? 'bg-green-100 text-green-700 border border-green-300 dark:bg-green-900/30 dark:text-green-300 dark:border-green-700' : 'bg-gray-100 text-gray-600 border border-gray-300 dark:bg-neutral-700 dark:text-gray-300 dark:border-neutral-600' }}">
                                {{ $procurement->approved_ppmp ? 'Yes' : 'No' }}
                            </span>
                        </td>
                        {{-- 16 APP (Updated) --}}
                        <td class="px-3 py-2 text-xs text-center text-gray-700 dark:text-gray-300">
                            <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-semibold {{ $procurement->app_updated ? 'bg-green-100 text-green-700 border border-green-300 dark:bg-green-900/30 dark:text-green-300 dark:border-green-700' : 'bg-gray-100 text-gray-600 border border-gray-300 dark:bg-neutral-700 dark:text-gray-300 dark:border-neutral-600' }}">
                                {{ $procurement->app_updated ? 'Yes' : 'No' }}
                            </span>
                        </td>
                        {{-- 17 Immediate Date Needed --}}
                        <td class="px-3 py-2 text-xs text-center text-gray-700 dark:text-gray-300 whitespace-nowrap">{{ $fmt($procurement->immediate_date_needed) }}</td>
                        {{-- 18 Date Needed --}}
                        <td class="px-3 py-2 text-xs text-center text-gray-700 dark:text-gray-300 whitespace-nowrap">{{ $fmt($procurement->date_needed) }}</td>
                        {{-- 19 PMO/End-User --}}
                        <td class="px-3 py-2 text-xs text-left text-gray-700 dark:text-gray-300">
                            <div class="break-words whitespace-normal line-clamp-2">{{ $procurement->endUser?->endusers ?? '' }}</div>
                        </td>
                        {{-- 20 EPA --}}
                        <td class="px-3 py-2 text-xs text-center text-gray-700 dark:text-gray-300">
                            <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-semibold {{ $procurement->early_procurement ? 'bg-blue-100 text-blue-700 border border-blue-300 dark:bg-blue-900/30 dark:text-blue-300 dark:border-blue-700' : 'bg-gray-100 text-gray-600 border border-gray-300 dark:bg-neutral-700 dark:text-gray-300 dark:border-neutral-600' }}">
                                {{ $procurement->early_procurement ? 'Yes' : 'No' }}
                            </span>
                        </td>
                        {{-- 21 Source of Funds --}}
                        <td class="px-3 py-2 text-xs text-left text-gray-700 dark:text-gray-300">
                            <div class="break-words whitespace-normal line-clamp-2">{{ $procurement->fundSource?->fundsources ?? '' }}</div>
                        </td>
                        {{-- 22 Expense Class --}}
                        <td class="px-3 py-2 text-xs text-center text-gray-700 dark:text-gray-300">{{ $procurement->expense_class }}</td>
                        {{-- 23 ABC --}}
                        <td class="px-3 py-2 text-xs text-right font-bold text-gray-900 dark:text-white whitespace-nowrap">
                            @if ($procurement->abc !== null)
                                <div class="inline-flex items-baseline gap-0.5">
                                    <span class="text-gray-500 dark:text-gray-400 font-normal">₱</span>
                                    <span class="text-emerald-700 dark:text-emerald-400">{{ number_format($procurement->abc, 2) }}</span>
                                </div>
                            @endif
                        </td>
                        {{-- 24 Mode of Procurement --}}
                        <td class="px-3 py-2 text-xs text-left text-gray-700 dark:text-gray-300">
                            <div class="break-words whitespace-normal line-clamp-2">{{ $latestMop?->modeOfProcurement?->modeofprocurements ?? ($isPerLot ? '' : 'Multiple') }}</div>
                        </td>
                        {{-- 25 ABC <=> 50k --}}
                        <td class="px-3 py-2 text-xs text-center text-gray-700 dark:text-gray-300">{{ $procurement->abc_50k }}</td>
                        {{-- 26 Pre-Proc Conference --}}
                        <td class="px-3 py-2 text-xs text-center text-gray-700 dark:text-gray-300 whitespace-nowrap">{{ $isBidding ? $fmt($mainBid?->pre_proc_conference) : '' }}</td>
                        {{-- 27 Ads/Post of IB --}}
                        <td class="px-3 py-2 text-xs text-center text-gray-700 dark:text-gray-300 whitespace-nowrap">{{ $isBidding ? $fmt($mainBid?->ads_post_ib) : ($isSvp ? $fmt($prSvp?->ads_post_ib) : '') }}</td>
                        {{-- 28 Pre-bid Conf --}}
                        <td class="px-3 py-2 text-xs text-center text-gray-700 dark:text-gray-300 whitespace-nowrap">{{ $isBidding ? $fmt($mainBid?->pre_bid_conf) : '' }}</td>
                        {{-- 29 Eligibility Check --}}
                        <td class="px-3 py-2 text-xs text-center text-gray-700 dark:text-gray-300 whitespace-nowrap">{{ $isBidding ? $fmt($mainBid?->eligibility_check) : '' }}</td>
                        {{-- 30 Sub/Open of Bids --}}
                        <td class="px-3 py-2 text-xs text-center text-gray-700 dark:text-gray-300 whitespace-nowrap">{{ $isBidding ? $fmt($mainBid?->sub_open_bids) : '' }}</td>
                        {{-- 31 1st Bidding Date --}}
                        <td class="px-3 py-2 text-xs text-center text-gray-700 dark:text-gray-300 whitespace-nowrap">{{ $isBidding ? $fmt($bidding1?->sub_open_bids) : '' }}</td>
                        {{-- 32 1st Bidding Result --}}
                        <td class="px-3 py-2 text-xs text-center text-gray-700 dark:text-gray-300">{{ $isBidding ? ($bidding1?->bidding_result ?? '') : '' }}</td>
                        {{-- 33 2nd Bidding Date --}}
                        <td class="px-3 py-2 text-xs text-center text-gray-700 dark:text-gray-300 whitespace-nowrap">{{ $isBidding ? $fmt($bidding2?->sub_open_bids) : '' }}</td>
                        {{-- 34 2nd Bidding Result --}}
                        <td class="px-3 py-2 text-xs text-center text-gray-700 dark:text-gray-300">{{ $isBidding ? ($bidding2?->bidding_result ?? '') : '' }}</td>
                        {{-- 35 Bid Evaluation Date --}}
                        <td class="px-3 py-2 text-xs text-center text-gray-700 dark:text-gray-300 whitespace-nowrap">{{ $isBidding ? $fmt($mainBid?->bid_evaluation_date) : '' }}</td>
                        {{-- 36 Post Qual Date --}}
                        <td class="px-3 py-2 text-xs text-center text-gray-700 dark:text-gray-300 whitespace-nowrap">{{ $isBidding ? $fmt($mainBid?->post_qualification_date) : '' }}</td>
                        {{-- 37 Resolution No. (Recom. Mode) --}}
                        <td class="px-3 py-2 text-xs text-center text-gray-700 dark:text-gray-300">{{ $isBidding ? ($mainBid?->resolution_number_mop ?? '') : ($isSvp ? ($prSvp?->resolution_number_mop ?? '') : '') }}</td>
                        {{-- 38 NTF No. (future) --}}
                        <td class="px-3 py-2 text-xs text-gray-300 dark:text-gray-600"></td>
                        {{-- 39 NTF Bidding Date (future) --}}
                        <td class="px-3 py-2 text-xs text-gray-300 dark:text-gray-600"></td>
                        {{-- 40 NTF Bidding Result (future) --}}
                        <td class="px-3 py-2 text-xs text-gray-300 dark:text-gray-600"></td>
                        {{-- 41 RFQ No. --}}
                        <td class="px-3 py-2 text-xs text-center text-gray-700 dark:text-gray-300">{{ $isSvp ? ($prSvp?->rfq_no ?? '') : '' }}</td>
                        {{-- 42 Canvass Date --}}
                        <td class="px-3 py-2 text-xs text-center text-gray-700 dark:text-gray-300 whitespace-nowrap">{{ $isSvp ? $fmt($prSvp?->canvass_date) : '' }}</td>
                        {{-- 43 Date Returned of Canvass --}}
                        <td class="px-3 py-2 text-xs text-center text-gray-700 dark:text-gray-300 whitespace-nowrap">{{ $isSvp ? $fmt($prSvp?->date_returned_of_canvass) : '' }}</td>
                        {{-- 44 Abstract of Canvass Date --}}
                        <td class="px-3 py-2 text-xs text-center text-gray-700 dark:text-gray-300 whitespace-nowrap">{{ $isSvp ? $fmt($prSvp?->abstract_of_canvass_date) : '' }}</td>
                        {{-- 45 Date of BAC Resolution (Award) --}}
                        <td class="px-3 py-2 text-xs text-center text-gray-700 dark:text-gray-300 whitespace-nowrap">{{ $fmt($post?->resolution_award_date) }}</td>
                        {{-- 46 Notice of Award Date --}}
                        <td class="px-3 py-2 text-xs text-center text-gray-700 dark:text-gray-300 whitespace-nowrap">{{ $fmt($post?->notice_of_award) }}</td>
                        {{-- 47 Awarded Amount --}}
                        <td class="px-3 py-2 text-xs text-right font-bold text-gray-900 dark:text-white whitespace-nowrap">
                            @if ($post?->awarded_amount !== null)
                                <div class="inline-flex items-baseline gap-0.5">
                                    <span class="text-gray-500 dark:text-gray-400 font-normal">₱</span>
                                    <span class="text-emerald-700 dark:text-emerald-400">{{ number_format($post->awarded_amount, 2) }}</span>
                                </div>
                            @endif
                        </td>
                        {{-- 48 Award Notice Number --}}
                        <td class="px-3 py-2 text-xs text-center text-gray-700 dark:text-gray-300">{{ $post?->notice_of_award_number ?? '' }}</td>
                        {{-- 49 Date of Posting of Award (PhilGEPS) --}}
                        <td class="px-3 py-2 text-xs text-center text-gray-700 dark:text-gray-300 whitespace-nowrap">{{ $fmt($post?->philgeps_posting_of_award) }}</td>
                        {{-- 50 Supplier --}}
                        <td class="px-3 py-2 text-xs text-left text-gray-700 dark:text-gray-300">
                            <div class="break-words whitespace-normal line-clamp-2">{{ $post?->supplier?->name ?? '' }}</div>
                        </td>
                        {{-- 51 Procurement Stage --}}
                        <td class="px-3 py-2 text-xs text-center text-gray-700 dark:text-gray-300">
                            @if ($procurement->currentPrStage?->procurementStage?->procurementstage)
                                <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-semibold bg-gradient-to-r from-blue-100 to-blue-200 text-blue-800 border border-blue-300 dark:from-blue-900/40 dark:to-blue-800/40 dark:text-blue-200 dark:border-blue-700 break-words whitespace-normal text-center">
                        ...
                        <td class="px-3 py-2 text-xs text-center text-gray-700 dark:text-gray-300">
                            @if ($remarkText)
                                <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium border break-words whitespace-normal text-center {{ $remarkBadge }}" title="{{ $remarkText }}">
                                    {{ $remarkText }}
                                </span>
                            @endif
                        </td>
                        {{-- 53 Remarks (Notes) --}}
                        <td class="px-3 py-2 text-xs text-left text-gray-700 dark:text-gray-300">
                            <div class="break-words whitespace-normal line-clamp-2">{{ $procurement->currentLotRemark?->notes ?? '' }}</div>
                        </td>
                        {{-- 54 Reschedule/Cancellation Letter (future) --}}
                        <td class="px-3 py-2 text-xs text-gray-300 dark:text-gray-600"></td>
                        {{-- 55 Date Forwarded to PMU --}}
                        <td class="px-3 py-2 text-xs text-center text-gray-700 dark:text-gray-300 whitespace-nowrap">{{ $fmt($pmu?->date_forwarded) }}</td>
                        {{-- 56 PhilGEPS Posting Ref No. --}}
                        <td class="px-3 py-2 text-xs text-center text-gray-700 dark:text-gray-300">{{ $isBidding ? ($mainBid?->philgeps_posting_ref_no ?? '') : ($isSvp ? ($prSvp?->philgeps_posting_ref_no ?? '') : '') }}</td>
                        {{-- 57 Contract Amount --}}
                        <td class="px-3 py-2 text-xs text-right font-bold text-gray-900 dark:text-white whitespace-nowrap">
                            @if ($pmuPo?->contract_amount !== null)
                                <div class="inline-flex items-baseline gap-0.5">
                                    <span class="text-gray-500 dark:text-gray-400 font-normal">₱</span>
                                    <span class="text-emerald-700 dark:text-emerald-400">{{ number_format($pmuPo->contract_amount, 2) }}</span>
                                </div>
                            @endif
                        </td>
                        {{-- 58 PO/Contract Number --}}
                        <td class="px-3 py-2 text-xs text-center text-gray-700 dark:text-gray-300">{{ $pmuPo?->po_contract_number ?? '' }}</td>
                        {{-- 59 Contract Signing/PO --}}
                        <td class="px-3 py-2 text-xs text-center text-gray-700 dark:text-gray-300 whitespace-nowrap">{{ $fmt($pmuPo?->contract_signing_date) }}</td>
                        {{-- 60 Notice to Proceed --}}
                        <td class="px-3 py-2 text-xs text-center text-gray-700 dark:text-gray-300 whitespace-nowrap">{{ $fmt($pmuPo?->notice_to_proceed_date) }}</td>
                        {{-- 61 Delivery/Completion --}}
                        <td class="px-3 py-2 text-xs text-center text-gray-700 dark:text-gray-300 whitespace-nowrap">{{ $fmt($supplyPo?->delivery_completion) }}</td>
                        ...
                        <td class="px-3 py-2 text-xs text-gray-300 dark:text-gray-600"></td>
                        {{-- 70 Remarks (Changes from APP, future) --}}
                        <td class="px-3 py-2 text-xs text-gray-300 dark:text-gray-600"></td>
                    </tr>
                @empty
                    <tr>
        ...
        <div class="flex flex-col items-center justify-center gap-3 flex-1">
            @if ($procurements->total() > 0)
                <div class="text-xs font-medium text-gray-600 dark:text-gray-300">
                    Showing <span class="text-emerald-600 dark:text-emerald-400 font-semibold">{{ $procurements->firstItem() }}</span>
                    to <span class="text-emerald-600 dark:text-emerald-400 font-semibold">{{ $procurements->lastItem() }}</span>
                    of <span class="text-emerald-600 dark:text-emerald-400 font-semibold">{{ $procurements->total() }}</span> records
                </div>
            @endif
